...
# Tarea asincrona de Celery para automatizar certificados
@celery.task
def automatizar_certificados(datos):
    automatizar_navegacion(datos)
...
def iniciar_automatizacion():
    ...
    task = automatizar_certificados.apply_async(args=[datos])
    return jsonify({"mensaje": "Automatización iniciada", "task_id": task.id}), 200
...
def descargar_resultados():
    ...
    # Comprimir archivo antes de enviarlo
    with zipfile.ZipFile('resultados_certificados.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
        zipf.write(archivo_resultados)
    
    return send_file('resultados_certificados.zip', as_attachment=True)
